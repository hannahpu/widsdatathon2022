...
def clean_impute_data(train_df, test_df, impute_col='year_built', impute_thresh=1800, factors_cols=['facility_type', 'state_factor']):
    agg_df = train_df.groupby(factors_cols)[impute_col].median(
...
def log_transform(df, cols_to_transform, log_type='log10'):
    ...
    df = df.rename(columns=dict(zip(cols_to_transform, [f"{log_type}_{col}" for col in cols_to_transform])))
    return df
...
def invert_log_transform(df, cols_to_invert, log_type='log10'):
    ...
    for col in cols_to_invert:
        df[col] = log_func[log_type](df[col]) - 1
    df = df.rename(columns=dict(zip(cols_to_invert, [col.replace(log_type, "") for col in cols_to_invert])))
    return df
...
def reduce_dimensions(train_x_df, test_x_df, cols_to_reduce, prefix='temp', n_components=9):
    ...
    scaled_train_x_df, scaled_test_x_df = scale_data(train_x_df[cols_to_reduce], test_x_df[cols_to_reduce])
...
def pca_fit(x, n_components, var_thresh=0.75, var_diff_thresh=0.01):
    ...
    if n_components <= 0 or None:
        n_components = get_pca_n_components(x, var_thresh=var_thresh, var_diff_thresh=var_diff_thresh)
...
def get_rmse_by_group(x_df, y_df, y_pred_df, group_cols=['facility_type']):
    ...
    rmse_by_group_df = merged_df.groupby(group_cols).agg(rmse=('y_diff', lambda x: np.sqrt((x**2).sum()/x.size)), 
